...
def test_classes_bound_and_unbound_methods():
    ...
    assert AttributeError,"Unbound method" == TestQueue.push.__self__   #unbound method
...
class Person:
    ...
    def get_bmi_result(self):
        self.height= (float(self.height)*0.305)
        self.w=(float(self.weight)/(self.height*self.height))
        if self.w<18.5:
            return "underweight"
        elif 18.5<self.w<24.9:
            return "healthy"
        elif 25<self.w<29.9:
            return "overweight"
        elif self.w>30:
