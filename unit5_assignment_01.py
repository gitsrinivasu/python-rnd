...
def de_dup_and_sort(input):
    ...
    if input==None:
        return None
    import string
    if (input[0] not in string.ascii_letters) and (len(input[0])<2):
...
#the list was not sorted so made the list sorted
    assert ['1', '2', '3'] == de_dup_and_sort(["1","3","1","2"])
    assert ['1', '2', '11', '23', '33'] == de_dup_and_sort(["1","23","1","11","2","33"])
