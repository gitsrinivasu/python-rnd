...
def test_function_call_with_keyword_arguments():
    assert [10, 30, 3, 4] == demo(10,30)#when given one parameter it is giving error so added second parameter also
    assert [10, 20, 3, 4] == demo(10, 20)
    assert [10,20,30,4] == demo(10, 20, 30)
    assert [45,20,3,4] == demo(first=45,second=20)#giving error saying required positional argument "first" ao added first argument
    assert [40,20,30,4] == demo(first=40,second=20, third=30)#giving error saying required positional argument "first" ao added first argument
    assert [10,60,30,4] == demo(first=10,second=60, third=30)#giving error saying required positional argument "second" ao added second argument
    assert [10,80,30,4] == demo(10,80, third=30)#giving error saying required positional argument "second" ao added second argument
    #assert [__] == demo(second=20, 10)  # is this allowed? correct and uncomment gives error positional argument follows keyword argument
...
def simple_format(format, *args):
    ...
    if format[0]=='%':
        if format[1]=='0':
            format=args[0]+format[2:]
        if format[1]=='1':
            format=args[1]+format[2:]

    if format[-2]=='%':
        if format[-1]=='0':
            format=format[:-3]+" "+args[0]
        if format[-1]=='1':
            format=format[:-3]+" "+args[1]

    for i in range(len(format)):
        if format[i] == '%':
            if format[i+1] == '0':
                format=format[:i-1]+" "+args[0]+format[i+2:]
            if format[i+1] == '1':
                format = format[:i - 1] + " "+args[1] + format[i + 2:]
