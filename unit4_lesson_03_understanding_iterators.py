...
def test_for():
    ...
    for x in iter1:
        result.append(x)
    assert [] == result  #what happened here?as .__next__ is pointing out of len range of list in iter1
...
def test_find_builtins_that_work_on_iterables():
    funcs = ["zip","sum","sorted","min","max","map","filter","enumerate","any","all"]
    assert 10 == len(funcs)
