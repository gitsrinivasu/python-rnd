...
def test_exception_basic():
    ...
    try:
        ...
        assert True, "sample error"#it was False made it True to pass the test
...
def test_exception_flow_1():
    ...
    try:
        fruit = fruit.upper()
        result.append("one")
        fruit.missingmethod() # what happens to the control flow here?#here there is an attribute error so control flow goes to  "except AttributeError as ae:"
        result.append("two")  # will this line get executed?#no this line will not be executed
...
def test_exception_flow_2():
    ...
    try:
        result.append("one")
        value = 1/0             #division by zero.#due to this the control flow goes to ZeroDivisionError
        result.append("two")
...
def test_nested_exceptions():
    ...
    except AssertionError as se:
        print(se)#it is giving error in finally
...
-about raising error and about Attribute,Airthmetic and Assertion error
-about finally block that is exceuted always besides taking take care whether the exception is raised or not 
-else clause in exception
"""
